...
class HwpController:
        ...
        try:
            ...
            if register_security_module:
                try:
                    ...
                    # hwp_controller.py 파일의 위치를 기준으로 상대 경로를 사용하여 절대 경로 생성
                    base_dir = os.path.dirname(os.path.abspath(__file__))
                    module_path = os.path.join(base_dir, "..", "..", "security_module", "FilePathCheckerModuleExample.dll")
                    
                    if os.path.exists(module_path):
                        self.hwp.RegisterModule("FilePathCheckerModuleExample", module_path)
                        print(f"보안 모듈이 등록되었습니다: {module_path}")
                    else:
                        print(f"보안 모듈을 찾을 수 없습니다. 경로를 확인하세요: {module_path}")
    ...
    def create_new_document(self) -> bool:
        ...
        try:
            ...
            self.current_document_path = "C:\\Users\\m1ns2o\\Desktop\\test"
            return True
        except Exception as e:
