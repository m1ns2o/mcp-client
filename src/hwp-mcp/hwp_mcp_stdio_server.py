...
def get_hwp_controller(force_new: bool = False):
    """Get or create HwpController instance."""
    global hwp_controller, hwp_table_tools

    if force_new and hwp_controller is not None:
        logger.info("Forcing new HwpController instance by closing the existing one.")
        try:
            hwp_controller.disconnect()
        except Exception as e:
            logger.warning(f"An error occurred while disconnecting the existing HwpController: {str(e)}")
        hwp_controller = None
        hwp_table_tools = None
...
@mcp.tool()
def hwp_open(path: str) -> str:
    """기존 HWP 문서를 엽니다. 파일명만 전달된 경우 프로젝트 루트에서 파일을 찾습니다."""
    try:
        if not path:
            return "Error: File path is required"

        # 1. 프로젝트 루트 경로 계산
        # 현재 스크립트 파일의 위치를 기준으로 상위 폴더를 프로젝트 루트로 간주합니다.
        # __file__은 현재 스크립트의 경로입니다.
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        
        # 2. 전달된 경로가 절대 경로인지 확인
        if os.path.isabs(path):
            # 이미 절대 경로이면 그대로 사용
            final_path = path
        else:
            # 상대 경로이면 프로젝트 루트와 조합하여 새로운 절대 경로 생성
            final_path = os.path.join(project_root, path)

        logger.info(f"전달된 경로: '{path}', 최종 처리 경로: '{final_path}'")

        # 3. 파일 존재 여부 확인
        if not os.path.exists(final_path):
            return f"Error: 파일을 찾을 수 없습니다: {final_path}"

        # 4. HWP 인스턴스 가져오기 및 문서 열기
        hwp = get_hwp_controller(force_new=True)
        if not hwp:
            return "Error: HWP 프로그램을 제어할 수 없습니다"
        
        logger.info(f"문서 열기 시도: {final_path}")
        if hwp.open_document(final_path):
            logger.info(f"문서를 성공적으로 열었습니다: {final_path}")
            return f"문서를 열었습니다: {os.path.basename(final_path)}"
        else:
            error_msg = f"Error: 문서 열기에 실패했습니다: {final_path}"
            logger.error(error_msg)
            return error_msg
            
    except Exception as e:
        logger.error(f"문서 열기 중 예외 발생: {str(e)}", exc_info=True)
        return f"Error: {str(e)}"
...
#     """Open an existing HWP document."""
#     try:
#         import os # 함수 상단으로 이동하면 더 좋습니다.

#         # ▼▼▼ 디버깅을 위한 로깅 추가 ▼▼▼
#         logger.info(f"hwp_open 호출됨. 전달받은 path: '{path}'")
#         logger.info(f"현재 작업 디렉토리(CWD): '{os.getcwd()}'")
#         if not path:
#             return "Error: File path is required"
...
#         logger.info(f"Attempting to open document: {abs_path}")
#         if hwp.open_document(abs_path):
#             logger.info(f"Successfully opened document: {abs_path}")
#             return f"Document opened: {abs_path}"
#         else:
#             error_msg = "Error: Failed to open document"
#             logger.error(error_msg)
#             return error_msg
#     except Exception as e:
#         logger.error(f"Error opening document: {str(e)}", exc_info=True)
...
if __name__ == "__main__" and test:
    def test_hwp_controller():
        """
        HwpController의 주요 기능을 테스트하는 함수
        """
        print("--- HwpController 테스트 시작 ---")
        
        # HWP 컨트롤러 인스턴스 가져오기
        hwp = get_hwp_controller(force_new=True)
        if not hwp:
            print("오류: 한글 프로그램을 시작할 수 없습니다.")
            return

        # 1. 새 문서 생성
        # print("\n[1] 새 문서 생성 테스트")
        # if hwp.create_new_document():
        #     print(" -> 성공: 새 문서가 생성되었습니다.")
        # else:
        #     print(" -> 실패: 새 문서를 생성하지 못했습니다.")
        #     hwp.disconnect()
        #     return

        # 2. 텍스트 삽입
        # print("\n[2] 텍스트 삽입 테스트")
        # text_to_insert = "이것은 HwpController 자동화 테스트입니다.\n두 번째 줄입니다."
        # if hwp.insert_text(text_to_insert):
        #     print(f" -> 성공: 다음 텍스트가 삽입되었습니다.\n'{text_to_insert}'")
        # else:
        #     print(" -> 실패: 텍스트를 삽입하지 못했습니다.")

        # # 3. 글꼴 설정
        # print("\n[3] 글꼴 설정 테스트")
        # if hwp.set_font_style(font_name="궁서", font_size=15, bold=True, select_previous_text=True):
        #     print(" -> 성공: 방금 삽입한 텍스트의 글꼴이 '궁서', 15pt, 굵게로 변경되었습니다.")
        # else:
        #     print(" -> 실패: 글꼴을 설정하지 못했습니다.")
        ...
        # # 4. 표 생성
        # print("\n[4] 표 생성 테스트")
        # if hwp.insert_table(3, 4):
        #     print(" -> 성공: 3x4 표가 생성되었습니다.")
        # else:
        #     print(" -> 실패: 표를 생성하지 못했습니다.")
        ...
        # ]
        # table_tools = get_hwp_table_tools()
        # if table_tools and table_tools.fill_table_with_data(table_data, has_header=True):
        #     print(" -> 성공: 표에 데이터가 채워졌습니다.")
        # else:
        #     print(" -> 실패: 표에 데이터를 채우지 못했습니다.")

        # # 6. 문서 저장
        # print("\n[6] 문서 저장 테스트")
        save_path = os.path.join(os.path.dirname(__file__), "test_document.hwp")
        # if hwp.save_document(save_path):
        #     print(f" -> 성공: 문서가 다음 경로에 저장되었습니다: {save_path}")
        # else:
        #     print(" -> 실패: 문서를 저장하지 못했습니다.")


        print("\n[7] 문서 오픈 테스트")
        if hwp.open_document(save_path):
            print(" -> 성공: 문서가 열렸습니다.")
        else:
            print(" -> 실패: 문서를 열지 못했습니다.")

        print("\n--- HwpController 테스트 종료 ---")
