...
class MCPClient {
    ...
    async start() {
        try {
            this.process = (0, child_process_1.spawn)(this.mcpServerPath, [], {
    ...
    getToolsSummary() {
        ...
        for (const tool of this.availableTools) {
            ...
            if (tool.inputSchema?.properties) {
                ...
                for (const [argName, argInfo] of Object.entries(tool.inputSchema.properties)) {
                    ...
                    argsInfo.push(`  - ${argName} (${argType})${requiredMark}: ${argDesc}`);
                }
            }
...
class LLMClient {
    llmServerUrl;
    client;
    constructor(llmServerUrl = 'http://localhost:8000') {
...
class HWPPromptProcessor {
    mcpClient;
    llmClient;
    constructor(mcpClient, llmClient) {
...
5. 문서 저장 → hwp_save`;
    ...
    async processRequest(userInput, conversationHistory = []) {
        // 시스템 프롬프트 추가 (맨 처음에만)
        ...
        if (toolCalls.length) {
            for (const toolCall of toolCalls) {
                try {
                    Logger.info(`도구 호출: ${toolCall.tool_name} with ${JSON.stringify(toolCall.arguments)}`);
                    const result = await this.mcpClient.callTool(toolCall.tool_name, toolCall.arguments);
                    // 결과 텍스트 추출
                    let resultText = '성공';
                    if (result.content) {
...
class HWPPromptClient {
    ...
    processor = null;
    conversationHistory = [];
    constructor(mcpServerPath, llmServerUrl = 'http://localhost:8000') {
    ...
    async processPrompt(prompt) {
        ...
        try {
            const [message, toolResults] = await this.processor.processRequest(prompt, this.conversationHistory);
            return {
                success: true,
                message,
    ...
    async runInteractive() {
        ...
        console.log("자연어로 HWP 작업을 요청해보세요. 'quit' 입력 시 종료됩니다.\n");
                ...
                if (result.success) {
                    ...
                    if (result.tool_results.length) {
                        ...
                        for (const toolResult of result.tool_results) {
                            ...
                            console.log(`  ${status} ${toolResult.description}: ${toolResult.result}`);
                        }
                    }
                }
                else {
